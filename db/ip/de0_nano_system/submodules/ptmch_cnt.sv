...
//=================================================================
    ...
    always_ff @(posedge CLK100M or negedge RESET_N) begin
        if(!RESET_N)
            ...
            case(PAGEADDR_SEL) 
                3'b000  : PADDR_CNT <= sr_prgexct_counter[ 6: 0]; // 0x0:program excute
                3'b001  : PADDR_CNT <= sr_rdstat_counter[ 6: 0];  // 0x1:readstatus
                3'b010  : PADDR_CNT <= sr_blkers_counter[ 6: 0];  // 0x2:128kb_blockerase
                3'b011  : PADDR_CNT <= sr_pdread_counter[ 6: 0];  // 0x3:pagedata_read
                3'b100  : PADDR_CNT <= sr_wrstat_counter[ 6: 0];  // 0x4:writestatus
                default : PADDR_CNT <= sr_prgexct_counter[ 6: 0];
            endcase
        end
    end
